...
def benchmark_all_devices():
    ...
    for i in range(device_count):
        ...
        time_taken = benchmark_device(opencl, i, test_size=1000000)
...
def benchmark_saxpy(n=1000000):
    ...
    # Transferir para o dispositivo
    d_x = opencl.to_device(x)
    d_y = opencl.to_device(y)
    d_result = opencl.to_device(result_opencl)
...
def benchmark_convolution_2d(width=1024, height=1024):
    ...
    # Transferir para o dispositivo
    d_image = opencl.to_device(image.flatten())
    d_filter = opencl.to_device(sobel_x.flatten())
    d_result = opencl.to_device(result_opencl.flatten())
...
def run_examples():
    ...
    d_x = opencl.to_device(x)
    d_y = opencl.to_device(y)
    d_result = opencl.to_device(result)
    ...
    d_image = opencl.to_device(image.flatten())
    d_filter = opencl.to_device(sobel_x.flatten())
    d_result = opencl.to_device(result_img.flatten())
